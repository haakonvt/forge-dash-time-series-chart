...
def update_select_time_series(_):  # TODO: How to get rid of input?
    ts = [get_client().time_series.retrieve(id=6879658344978017)]
    options = [
        {"label": name, "value": val}
        for name, val in zip([t.name for t in ts], [t.external_id for t in ts])
    ]
    return options
...
inp_grp_ts_selector = dbc.InputGroup(
    [
        ...
        dbc.Select(id="select-ts", options=[], bs_size="lg"),
    ]
)
inp_grp_date_selector = dbc.InputGroup([
    dbc.InputGroupAddon("Date range: ", addon_type="prepend"),
    dcc.DatePickerRange(id="select-dates", start_date="2020-11-20", end_date="2020-11-23"),
])

# Full app layout:
app.layout = html.Div([
    html.Div(id="time-filter", style={"display": "none"}),
    dbc.Card(
        [
